...
function build_dep() {
	...
	PYTHONPATH=${BUILD_APPDIR}/usr/lib/python3.6/site-packages python3 \
		setup.py install --optimize=1 \
		--prefix="/usr/" --root="${BUILD_APPDIR}"
	mkdir -p "${BUILD_APPDIR}/usr/lib/python3.6/site-packages/"
...
# Prepare & build deps
export PYTHONPATH=${BUILD_APPDIR}/usr/lib/python3.6/site-packages/
mkdir -p "$PYTHONPATH"
if [[ $(grep ID_LIKE /etc/os-release) == *"suse"* ]] ; then
	# Special handling for OBS
	ln -s lib64 ${BUILD_APPDIR}/usr/lib
	export PYTHONPATH="$PYTHONPATH":${BUILD_APPDIR}/usr/lib64/python3.6/site-packages/
	LIB=lib64
fi
...
cp "/usr/include/linux/input-event-codes.h" ${BUILD_APPDIR}/usr/${LIB}/python3.6/site-packages/scc/
